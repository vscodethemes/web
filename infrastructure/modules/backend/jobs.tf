...
module "extract_themes" {
  ...
  sqs_send_arns    = ["${aws_sqs_queue.extract_themes.arn}", "${aws_sqs_queue.extract_themes_deadletter.arn}", "${aws_sqs_queue.save_theme.arn}"]
  sqs_delete_arns  = ["${aws_sqs_queue.extract_themes.arn}"]
  sns_publish_arns = ["${aws_sns_topic.extract_themes.arn}", "${aws_sns_topic.save_theme.arn}"]
